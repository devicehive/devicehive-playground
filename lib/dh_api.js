...
var api = function (config) {
            ...
            request.get(path('network', {name: networkName}), {
                ...
                if (response && response.length) {
                    ...
                    self.createNetwork(networkName, data.jwt).then(function (response) {
                        ...
                        return resolve(_.extend(data, {network: newNetwork, networkIds: [response.id]}));
                    })['catch'](reject);
                }
            })['catch'](reject);
        });
    ...
    self.getJWT = function (data) {
        return new Promise(function (resolve, reject) {
            ...
            var token = {
                userId: data.user.id,
                deviceTypeIds: ['*'] // all device types available by default
            };

            if (data.user.role === 0) {
                token.actions = ['*']; // all actions for admin
                token.networkIds = ['*'];
            } else {
                token.actions = ['GetNetwork'
                    , 'GetDeviceType'
                    , 'GetDevice'
                ...
                token.networkIds = data.networkIds;
            }
