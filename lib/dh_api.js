...
var api = function (config) {
    ...
    var path = function (path, query) {
        if (query) {
            var queryString = Object.keys(query)
                ...
                .join('&');
            return `${self.api_url}/${path}?${queryString}`;
        } else {
    ...
    var ROLE_ADMIN = 0, ROLE_CLIENT = 1;

    self.getUserByEmail = function (email) {
        return new Promise(function (resolve, reject) {

            // get user by email
            request.get(path('user', {login: email}), {headers: auth(self.api_key), json: true})
                .then(response => {
                    ...
                    self.createUser(email)
                        .then(response => resolve({user: response, email: email}))
                        .catch(reject)
                })
                .catch(reject);
    ...
    self.getNetwork = function (data) {
        return new Promise(function (resolve, reject) {
            ...
            // get network
            request.get(path(`network`, {name: networkName}), {headers: auth(self.api_key), json: true})
                .then(response => {
                    if (response && response.length) {
                        return resolve(_.extend(data, {network: response[0]}));
                    }
                    console.log(`User ${data.email} has no network key yet. Creating...`);
                    self.createNetwork(networkName, networkKey)
                        .then(response => resolve(_.extend(data, {network: response})))
                        .catch(reject)
                })
                .catch(reject);
    ...
    self.getAccessKey = function (data) {
        return new Promise(function (resolve, reject) {
            ...
            // get accesskey
            request.get(path(`user/${data.user.id}/accesskey`), {headers: auth(self.api_key), json: true})
                .then(response => {
                    if (response && response.length) {
                        return resolve(_.extend(data, {accessKey: response[0]}));
                    }
                    console.log(`User ${data.email} has no access key yet. Creating...`);
                    self.createAccessKey(data.user.id, data.email, data.network.id)
                        .then(response => resolve(_.extend(data, {accessKey: response})))
                        .catch(reject)
                })
                .catch(reject);
    ...
    self.createAccessKey = function (user_id, email, netowrk) {
        var token = {
            type: ROLE_CLIENT,
        ...
        return request.post(path(`user/${user_id}/accesskey`), {headers: auth(self.api_key), body: token, json: true});
    };
