    ...
    self.getAdminToken = function () {
        return new Promise(function (resolve, reject) {
            ...
            request.post(path('token'), {
                ...
                return resolve({jwt: response.accessToken});
            })['catch'](reject);
        });
    };

    self.getUserByEmail = function (email, data) {
        return new Promise(function (resolve, reject) {
            if (!data || !data.jwt) {
            ...
            request.get(path('user', {login: email}), {
                ...
                self.createUser(email, data.jwt).then(function (response) {
                    return resolve(_.extend(data, {user: response, email: email}));
                })['catch'](reject);
            })['catch'](reject);
        });
    };
