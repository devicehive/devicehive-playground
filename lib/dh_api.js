...
var api = function (config) {
    ...
    self.getUserByEmail = function (email) {
        return new Promise(function (resolve, reject) {
            ...
            // get user by email
            request.get(path('user', { login: email }), { headers: auth(self.api_key), json: true }).then(function (response) {
    ...
    self.getNetwork = function (data) {
        return new Promise(function (resolve, reject) {
            ...
            // get network
            request.get(path('network', { name: networkName }), { headers: auth(self.api_key), json: true }).then(function (response) {
                if (response && response.length) {
    ...
    self.getAccessKey = function (data) {
        return new Promise(function (resolve, reject) {
            ...
            // get accesskey
            request.get(path('user/' + data.user.id + '/accesskey'), { headers: auth(self.api_key), json: true }).then(function (response) {
                if (response && response.length) {
                    return resolve(_.extend(data, { accessKey: response[0] }));
                }
                console.log('User ' + data.email + ' has no access key yet. Creating...');
                self.createAccessKey(data.user.id, data.email, data.network.id).then(function (response) {
